  ...
  <div class="lesson">
    <LessonShell title="单元格读写" :ready="sheet !== null">
      <template #description>
        <p>
          <code>getCell(row, col).value()</code> /
          <code>setValue</code> 相当于对 <code>ref</code> 读写。<code
            >text()</code
          >
          是显示字符串，可能和底层 value 不同（例如日期）。
        </p>
        <p>
          <code>setArray</code> 一次写入一块矩形区域。<code
            >getSelections()</code
          >
          返回当前选区（零基行列）。
        </p>
      ...
      <template #actions>
        <button :disabled="sheet === null" type="button" @click="writeSample">
          写入样例数据块
        </button>
        <button :disabled="sheet === null" type="button" @click="readA1">
          读取 A1
        </button>
        <button :disabled="sheet === null" type="button" @click="readSelection">
          读取当前选区
        </button>
      </template>
    </LessonShell>
